...
{
    public Task<ShortenedUrl?> FindByOriginalUrlAsync(
        string originalUrl,
        CancellationToken cancellationToken
    ) =>
        db
            .ShortenedUrls.AsNoTracking()
            .FirstOrDefaultAsync(link => link.OriginalUrl == originalUrl, cancellationToken);

    public Task<ShortenedUrl?> FindByShortCodeAsync(
        string shortCode,
        CancellationToken cancellationToken
    ) =>
        db.ShortenedUrls.FirstOrDefaultAsync(
            link => link.ShortCode == shortCode,
            cancellationToken
        );

    public async Task<IReadOnlyList<ShortenedUrl>> GetRecentAsync(
        int limit,
        CancellationToken cancellationToken
    ) =>
        await db
