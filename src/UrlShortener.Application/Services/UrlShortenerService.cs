...
) : IUrlShortenerService
{
    ...
    )
    {
        ...
        for (var attempt = 0; attempt < 5; attempt++)
        {
            ...
            if (shortUrl.Length >= originalUrl.Length)
                throw new ArgumentException("This URL is already too short to shorten.");
            ...
            if (await repository.TryCreateAsync(link, cancellationToken))
    ...
    public async Task<LinkDetailResponse?> GetAsync(
        string code,
        string baseUrl,
        CancellationToken cancellationToken
    ) =>
        (await repository.FindByShortCodeAsync(code, cancellationToken)) is { } link
            ? ToDetail(link, baseUrl)
            : null;

    public async Task<IReadOnlyList<LinkDetailResponse>> GetRecentAsync(
        int limit,
        string baseUrl,
        CancellationToken cancellationToken
    ) =>
        (await repository.GetRecentAsync(Math.Clamp(limit, 1, 50), cancellationToken))
            .Select(link => ToDetail(link, baseUrl))
            .ToList();

    private static ShortUrlResponse ToResponse(ShortenedUrl link, string baseUrl) =>
        new(link.OriginalUrl, link.ShortCode, $"{baseUrl.TrimEnd('/')}/{link.ShortCode}");

    private static LinkDetailResponse ToDetail(ShortenedUrl link, string baseUrl) =>
        new(
            link.OriginalUrl,
            link.ShortCode,
